            ...
            if (attributes.is_table()) {
                for (const auto& [key, value] : attributes.as_table()) {
                    if (value.is_floating()) {
                        scripting::call(scripting_ctx, "set_attribute_value", key, value.as_floating());
                    } else if (value.is_integer()) {
                        scripting::call(scripting_ctx, "set_attribute_value", key, value.as_integer());
                    } else if (value.is_string()) {
                        scripting::call(scripting_ctx, "set_attribute_value", key, value.as_string().str);
                    } else if (value.is_boolean()) {
                        scripting::call(scripting_ctx, "set_attribute_value", key, value.as_boolean());
                    } else if (value.is_table()) {
                        for (const auto& [subkey, value] : value.as_table()) {
                            if (value.is_floating()) {
                                scripting::call(scripting_ctx, "set_attribute_value", key, subkey, value.as_floating());
                            } else if (value.is_integer()) {
                                scripting::call(scripting_ctx, "set_attribute_value", key, subkey, value.as_integer());
                            } else if (value.is_string()) {
                                scripting::call(scripting_ctx, "set_attribute_value", key, subkey, value.as_string().str);
                            } else if (value.is_boolean()) {
                                scripting::call(scripting_ctx, "set_attribute_value", key, subkey, value.as_boolean());
                            }
                        }
                    }
                }
