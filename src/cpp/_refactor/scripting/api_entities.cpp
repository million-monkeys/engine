...
extern "C" std::uint32_t entity_create (scripting::Context* context)
{
    ...
    entt::registry& registry = scenes::registry(context->m_scene_ctx);
...
{
    EASY_FUNCTION(scripting::COLOR(3));
    return magic_enum::enum_integer(scenes::loadEntity(context->m_scene_ctx, entt::hashed_string{prototype}));
}
...
{
    EASY_FUNCTION(scripting::COLOR(3));
    return magic_enum::enum_integer(scenes::findEntity(context->m_scene_ctx, entt::hashed_string{name}));
}
...
{
    return scenes::isInGroup(context->m_scene_ctx, static_cast<entt::entity>(entity), entt::hashed_string::hash_type(group));
}
